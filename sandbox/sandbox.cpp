...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    BPlusStoreType* ptrTree = new BPlusStoreType(14);
    ptrTree->template init<DataNodeType>();
    ...
    int* __i = new int(0);
   uintptr_t _p = reinterpret_cast<uintptr_t>(__i); 
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();

/*    for (size_t nCntr = 0; nCntr < 50000000; nCntr=nCntr+1)
    {
    void* __v = reinterpret_cast<void*>(_p);
    void* __w = reinterpret_cast<void*>(_p);

    }*/

for (size_t nCntr = 50000000; nCntr < 100000000; nCntr=nCntr+2)
    //for (size_t nCntr = 0; nCntr < 50000000; nCntr=nCntr+2)
    {
        ptrTree->insert(nCntr, nCntr);
    }

   // ptrTree->insert(500000001,2,true);
   for (size_t nCntr = 50000001; nCntr < 100000000; nCntr=nCntr+2)
// for (size_t nCntr = 1; nCntr < 50000000; nCntr=nCntr+2)
    {
        ptrTree->insert(nCntr, nCntr);
    }
